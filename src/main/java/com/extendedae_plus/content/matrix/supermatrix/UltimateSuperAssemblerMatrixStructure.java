...
public final class UltimateSuperAssemblerMatrixStructure {
    ...
    private static @Nullable List<ExpectedBlock> loadDefinition(ResourceManager resourceManager) {
        ...
        if (resource.isEmpty()) {
            ExtendedAEPlus.LOGGER.error("找不到终极超级装配矩阵结构文件: {}", DEFINITION);
            return null;
        }
        ...
        try (var reader = resource.get().openAsReader()) {
            ...
            if (size.get(0).getAsInt() != SIZE_X || size.get(1).getAsInt() != SIZE_Y
                ...
                ExtendedAEPlus.LOGGER.error("终极超级装配矩阵结构尺寸不正确");
                return null;
            }
            ...
            for (var element : root.getAsJsonArray("blocks")) {
                ...
                if (x < 0 || x >= SIZE_X || y < 0 || y >= SIZE_Y || z < 0 || z >= SIZE_Z) {
                    ExtendedAEPlus.LOGGER.error("终极超级装配矩阵结构包含越界坐标");
                    return null;
                }
                int index = x + z * SIZE_X + y * SIZE_X * SIZE_Z;
                if (occupied[index]) {
                    ExtendedAEPlus.LOGGER.error("终极超级装配矩阵结构包含重复坐标");
                    return null;
                }
                occupied[index] = true;
