...
public class UltimateSuperAssemblerMatrixBuilderItem extends Item {
    ...
    private static GlobalPos getStoredPosition(ItemStack stack, String tagKey) {
        ...
        CompoundTag positionTag = tag.getCompound(tagKey);
        if (!positionTag.contains(TAG_DIMENSION, Tag.TAG_STRING) || !positionTag.contains(TAG_POS, Tag.TAG_LONG)) {
            return null;
        }
        return GlobalPos.of(ResourceKey.create(Registries.DIMENSION,
                new ResourceLocation(positionTag.getString(TAG_DIMENSION))), BlockPos.of(positionTag.getLong(TAG_POS)));
    }
    ...
    private static void setStoredPosition(ItemStack stack, String tagKey, GlobalPos origin) {
        ...
        positionTag.putString(TAG_DIMENSION, origin.dimension().location().toString());
        positionTag.putLong(TAG_POS, origin.pos().asLong());
        tag.put(tagKey, positionTag);
    }

    private static void clearSelectedOrigin(ItemStack stack) {
        stack.getOrCreateTag().remove(TAG_SELECTED_ORIGIN);
    }
