...
public final class MatrixFrameBakedModel implements IDynamicBakedModel {
    ...
    private static BakedQuad bakeFace(Direction side, FaceBasis face, TextureAtlasSprite sprite, int rotation) {
        ...
        putVertex(builder, sprite, normal, face.bottomLeft(), rotateU(0, 1, rotation), rotateV(0, 1, rotation));
        putVertex(builder, sprite, normal, face.bottomRight(), rotateU(1, 1, rotation), rotateV(1, 1, rotation));
        putVertex(builder, sprite, normal, face.topRight(), rotateU(1, 0, rotation), rotateV(1, 0, rotation));
    ...
    private static void putVertex(QuadBakingVertexConsumer builder, TextureAtlasSprite sprite, Vec3i normal,
        ...
        // 使用浮点重载保持原始贴图颜色；整数 1 会被当作 1/255，导致模型近乎纯黑。
